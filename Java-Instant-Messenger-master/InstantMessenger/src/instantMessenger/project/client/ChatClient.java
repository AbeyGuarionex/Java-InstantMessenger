...
import instantMessenger.project.server.User;
...
public class ChatClient {
	...
	public static void connect() throws IOException{
		try {
			...
			showErrorDialog("Wrong class returned.");
			e.printStackTrace();
		} catch (UnknownHostException e) {
			System.err.println("Host " + hostName + " and Port " + portNumber
					+ " could not be resolved.");
			showErrorDialog("Host " + hostName + " and Port " + portNumber + " could not be resolved.");
			e.printStackTrace();
		} catch (IOException e) {
			System.err.println("Unable to establish I/O connection with " +
					hostName + " and Port " + portNumber);
			  showErrorDialog("Unable to establish I/O connection with " + hostName + " and Port " + portNumber);
			e.printStackTrace();
		}
	}	
	
	private static void showErrorDialog(String errorMessage) {
	    JOptionPane optionPane = new JOptionPane(errorMessage, JOptionPane.ERROR_MESSAGE, JOptionPane.DEFAULT_OPTION);
	    JDialog dialog = optionPane.createDialog("Error");

	    // Add a property change listener to the option pane
	    optionPane.addPropertyChangeListener((evt) -> {
	        if ("value".equals(evt.getPropertyName())) {
	            // Check if the OK button is pressed
	            if ((int) evt.getNewValue() == JOptionPane.OK_OPTION) {
	                // Close the CCView frame
	                view.closeFrame();
	            }
	        }
	    });
	...
	public static boolean checkIfUserNameExists(String[] userArray, String userName) {
